...
@endif
  ...
  <h1 class="h4 fw-bold mb-0">Edit Department</h1>
  <a class="btn btn-sm btn-outline-secondary" href="{{ route('departments.index') }}"><i class="bi bi-arrow-left"></i> Back</a>
</div>

<form method="POST" action="{{ route('departments.update', $department->id) }}" class="card shadow-sm">
  @csrf
